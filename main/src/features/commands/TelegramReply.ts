...
export class TelegramReply {
    ...
    async send(chatId: string | number, text: string, threadId?: number, raw?: any): Promise<void> {
        try {
            ...
            const rawId =
                typeof rawIdCandidate?.id === 'number' ? rawIdCandidate.id
                    : typeof rawIdCandidate?.messageId === 'number' ? rawIdCandidate.messageId
                        : typeof rawIdCandidate?.msgId === 'number' ? rawIdCandidate.msgId
                            : undefined;
